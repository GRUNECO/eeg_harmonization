...
#    for allb in b:  
    ...
    if space == 'ic':
        ...
        pass
    datasova=G1G2(dataAll,dd,database_database,database=True)
    sova_name = 'Data_complete_'+space+'_sovaharmony_G1G2_'+allm
    save_complete(sova_name,datasova,path_feather)

    my_dataAll = np.array(dataAll)
    nmy_dataAll =negativeTest(my_dataAll)
    data_transformtdataAll = np.log(0.001+my_dataAll)
    # run harmonization and store the adjusted data
    my_modeldataAll, my_data_adjdataAll = harmonizationLearn(data_transformtdataAll, covarsAll)
    ...
    ##
    datacol=G1G2(new_dataAll)
    new_name = 'Data_complete_'+space+'_neuroHarmonize_G1G2_'+allm
    save_complete(new_name,datacol,path_feather)
    ##
