...
from funtionsHarmonize import to_save_df
...
#for allb in b:  
#Tab
data = pd.read_feather(path+'\Data_complete_roi.feather')
data = data[data['age'] >= 44] # (Controles 44) - (G1G2 24)
data = data[data['age'] <= 57] # (Controles 57) - (G1G2 38)
name = 'CTR_44to57'
first_columns = data.columns
data,covars = mapsDrop(data,0.0)
data.reset_index(drop=True).to_feather('{path}\{name}.feather'.format(path=path,name=name))
...
#    new_df = to_save_df(new_data,df)
#else:
#    new_df = to_save_df(new_data,new_df)
for band in columnas:
    ...
    plt.legend()
    plt.show()
    #plt.savefig(r'{path}\Transformados\{name}_{title}_density2538.png'.format(path=path,name=band,title=title))
    #plt.close()
    #if nh==0: 
    #    plt.savefig(r'{path}\SinNegativos\{name}_{title}_density2538.png'.format(path=path,name=band,title=title))
    #    plt.close()
    #elif nh!=0:
    #    plt.savefig(r'{path}\ConNegativos\{name}_{title}_density2538.png'.format(name=band,title=title))
    #    plt.close()
#Tab
...
writer.save()
#writer_trans.save()
new_name = 'NeuroHarmonize_CTR_44to57'
## Tab
writer.close()
#writer_trans.close()    