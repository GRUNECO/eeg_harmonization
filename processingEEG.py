...
for dataset in THE_DATASETS:
    ...
    for i in metricas:
        for j in spatial_matrix:
            ...
            """Conversion of dataframes to perform the df['database']=[name]*len(list_subjects)different SL, coherence, entropy, cross frequency, etc. graphs"""
            if i=='power':
            ...
            #     columns_cross_ic = [palabra for palabra in list(data_IC.keys()) if palabra.startswith("crossfreq")]

            # #     #dataframe_long_cross_roi(data_ROI,type='Cross Frequency',columns=columns_cross_roi,name="data_long_crossfreq_roi",path=path)
            #     dataframe_long_cross_ic(data_IC,type='Cross Frequency',columns=columns_cross_ic,name="data_long_crossfreq_components",path=path,spatial_matrix=j)
