...
icc_data=pd.read_csv(r'paquetes\eeg_harmonization\sovaharmony\Reproducibilidad\icc_values.csv',sep=';')
...
def barplot_icc(icc_data,group,plot=False,save=False):
    for band in bands:
        ...
        if save==True:
            plt.savefig('paquetes\eeg_harmonization\sovaharmony\Reproducibilidad\ICC\ICC_{name_group}_{name_band}_components.png'.format(name_group=group,name_band=band))
            plt.close()
        if plot:
            plt.show()
...
def barplot_icc_nB_1G(icc_data,group,plot=False,save=False):
    ...
    if save==True:
        plt.savefig('paquetes\eeg_harmonization\sovaharmony\Reproducibilidad\ICC\ICC_{name_group}_components.png'.format(name_group=group))
        plt.close()
    if plot:
        plt.show()
