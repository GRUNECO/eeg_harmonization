...
def get_coherence_freq(signal,window=3):
    '''
    Parameters
    ----------
        signal:mne.Epochs
...
def get_coherence_bands(signal,bands,window=3,freqs=None,Cfxy=None):
    if not freqs and not Cfxy and window:
        freqs,Cfxy = get_coherence_freq(signal,window)
...
def get_coherence(signal,bands,window):
    freqs,Cfxy = get_coherence_freq(signal,window)
    _,Cbxy = get_coherence_bands(signal,bands,freqs=freqs,Cfxy=Cfxy,window=None)
    return freqs,Cfxy,bands,Cbxy