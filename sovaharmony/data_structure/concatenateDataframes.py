    ...
    for space in s:
        ...
        for met in m:
            if space=='ROI':
                ...
                path_='{path}/derivatives/data_long/{space}/data_{task}_{metric}_long_{name_db}_components.feather'.format(path=dataset['input_path'],space=space,task=dataset['layout']['task'],metric=met,name_db=dataset['name']).replace('\\','/')
